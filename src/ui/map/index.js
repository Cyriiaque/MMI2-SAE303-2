...
Leaflet.render = function (
  ...
  for (const departmentCode in PostalGeopoint) {
    ...
    marker.postBacCount = markerData.count; // Stocker les candidatures post-bac pour chaque marqueur
    marker.bindPopup(
      "<b>" +
  ...
  markers.on("clusterclick", function (event) {
    ...
    let totalSTI2D = 0;
    let totalAutres = 0;
    let totalPostBac = 0; // Initialisation du total des candidatures post-bac

    childMarkers.forEach((marker) => {
      if (marker.options.color === "red") {
        // Marqueurs post-bac (cercles rouges)
        totalPostBac += marker.postBacCount || 0;
      } else {
        // Marqueurs des lycées
