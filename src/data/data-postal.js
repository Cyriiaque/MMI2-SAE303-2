...
Postal.getPostalGeopoint = function (dataPostal, dataCandidatures) {
  ...
  // Étape 1 : Comptage des candidatures par préfixe
  candidaturesPostal.forEach((code) => {
    const prefix = code.slice(0, 2); // Extraire les 2 premiers caractères avec slice
    result[prefix] = result[prefix] || {
      count: 0,
      geopoint: [],
      nom_de_la_commune: [],
    }; // Initialiser si non existant
    result[prefix].count++; // Incrémenter le compteur
  });

  // Étape 2 : Associer les _geopoint et nom_de_la_commune
  for (const key in postalCommune) {
    if (postalCommune.hasOwnProperty(key)) {
      const postalEntry = postalCommune[key];
      const prefix = postalEntry.code_postal.slice(0, 2); // Extraire les 2 premiers caractères avec slice
      if (result[prefix]) {
        result[prefix].geopoint = postalEntry._geopoint;
        result[prefix].nom_de_la_commune = postalEntry.nom_de_la_commune;
      }
    }
  }
